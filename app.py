...
# Limpar o cache antes de carregar os dados
st.cache_data.clear()
...
if modo == "Colaborador":
    ...
    if st.session_state.admin_authenticated:
        ...
        nova_resposta = st.text_area("Digite a resposta correspondente:", height=200)
        ...
        if pergunta_selecionada:
            ...
            resposta_editada = st.text_area("Editar Resposta:", str(df.at[indice, "Resposta"]) + "\n", height=200)
            palavras_chave_editadas = st.text_input("Editar Palavras-Chave (separadas por vírgula):", ", ".join(df.at[indice, "Palavras-Chave"]))
            if st.button("Salvar Alterações"):
                df.at[indice, "Pergunta"] = pergunta_editada
                df.at[indice, "Resposta"] = resposta_editada.strip()
